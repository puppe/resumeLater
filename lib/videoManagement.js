...
// Vimeo
siteHandlers.vimeo = new SiteHandler();

siteHandlers.vimeo.play = function(video) {
	tabs.open({
		url : "http://vimeo.com/" + vidGetId(video.vid),
		onReady : function(tab) {
			var worker = tab.attach({
				contentScriptFile : data.url("videoManagement/vimeo.js")
			});
			worker.port.emit('play', video.time);
		}
	});
};

siteHandlers.vimeo.retrieveTime = function(tab) {
	worker = tab.attach({
		contentScriptFile : data.url("videoManagement/vimeo.js")
	})
	worker.port.emit('time');
	worker.port.on('time', function(time) {
		siteHandlers.vimeo.port.emit('time', {tab : tab, time : time});
		worker.destroy();
	});
};

siteHandlers.vimeo.test = function(domain) {
	const regex = /^(.*\.)?vimeo.com$/;
	return regex.test(domain);
};

siteHandlers.vimeo.urlToVid = function(url) {
	if (url.indexOf("couchmode") != -1)
		return undefined;
	var site = "vimeo";
